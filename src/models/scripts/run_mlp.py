...
def main():
    parser = argparse.ArgumentParser(description="运行 MLP 回测实验")
    ...
    # MLP 超参数
    parser.add_argument("--hidden_sizes", type=str, default="64,32",
                        help="隐藏层大小，逗号分隔")
    ...
    print(f"模型参数: hidden_sizes={hidden_sizes}, dropout={args.dropout}")
    print(f"训练参数: epochs={args.epochs}, lr={args.learning_rate}")
    ...
    if args.save_results:
        save_path = manager.save_results("mlp")
        print(f"\n结果已保存: {save_path}")
